...
<head>
    ...
    <title>Tambah Kuis - SiEdukasi</title>
...
<body class="bg-slate-50 text-slate-700 font-sans antialiased flex items-center justify-center min-h-screen py-10">
    ...
    <div class="w-full max-w-2xl bg-white p-8 rounded-3xl shadow-lg border border-slate-100 mx-4">
        <h1 class="text-2xl font-bold text-slate-800 mb-2">✨ Buat Kuis Baru</h1>
        <p class="text-slate-500 mb-6 text-sm">Formulir khusus Admin untuk menambahkan kuis evaluasi baru.</p>
        ...
        <form action="{{ route('kuis.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="judul_kuis" required placeholder="Contoh: Kuis Dasar PHP"
            ...
            <div>
                ...
                <textarea name="deskripsi_kuis" rows="3" required placeholder="Tulis instruksi pengerjaan kuis..."
                    class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:ring-2 focus:ring-purple-400 focus:border-purple-400 outline-none transition-all"></textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-600 mb-2">Jumlah Soal</label>
                    <input type="number" name="jumlah_soal" required min="1" placeholder="Contoh: 10"
                        class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:ring-2 focus:ring-purple-400 focus:border-purple-400 outline-none transition-all">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-600 mb-2">Batas Waktu (Menit)</label>
                    <input type="number" name="waktu_menit" required min="1" placeholder="Contoh: 15"
                        class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:ring-2 focus:ring-purple-400 focus:border-purple-400 outline-none transition-all">
                </div>
            </div>
